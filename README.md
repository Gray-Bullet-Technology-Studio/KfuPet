<div align="center">
<img alt="logo" height="100" width="100" src="docs/logo.png" />
<h2> KfunPet </h2>
<p> 你的智能桌面伙伴 </p>
<br />

[![Stars](https://img.shields.io/github/stars/Lrht-llw/KfuPet?style=flat)](https://github.com/Lrht-llw/KfuPet/stargazers)
[![Version](https://img.shields.io/github/v/release/Lrht-llw/KfuPet)](https://github.com/Lrht-llw/KfuPet/releases)
[![License](https://img.shields.io/github/license/Lrht-llw/KfuPet)](https://github.com/Lrht-llw/KfuPet/blob/main/LICENSE)
[![Issues](https://img.shields.io/github/issues/Lrht-llw/KfuPet)](https://github.com/Lrht-llw/KfuPet/issues)

</div>

## 说明

> [!IMPORTANT]
>
> ### 关于本项目
>
> - 本项目 **KfuPet** 是一款基于 **WPF** 开发的桌面宠物应用
> - 采用 **混合动作系统**，结合本地预设动画与 AI 实时驱动，实现低成本、高性能且富有创意的角色交互表现
> - 本项目采用 **AGPL-3.0** 协议 + 自定协议进行开源

欢迎各位大佬 `Star` 😍

<br />

## 💻 更新记录

### v0.0.1版本待发布

<br />

## 🍪 关于画饼

### 暂时还没开发完，所以说画不了饼了 😅

<br />

## 🧑‍💻 开发

- 本项目采用 **C#** + **WPF** + **WUI** 开发
- .NET 版本要求：>= 8.0

### 快速开始

1. 使用 Visual Studio 打开项目
2. 还原 NuGet 依赖
3. 启动调试：`F5`
4. 构建：右键项目 → 生成

<br />

## 📜 开源许可

- **本项目仅供个人学习研究使用，禁止用于商业及非法用途**
- 本项目基于 [GNU Affero General Public License (AGPL-3.0)](https://www.gnu.org/licenses/agpl-3.0.html) + 自定协议进行开源
  1. **修改和分发：** 任何对本项目的修改和分发都必须基于 AGPL-3.0 进行，源代码必须一并提供
  2. **网络服务：** 如果本项目的代码被用于提供网络服务，服务的用户必须能够获取完整的源代码
  3. **派生作品：** 任何派生作品必须同样采用 AGPL-3.0，并在适当的地方注明原始项目的许可证
  4. **免责声明：** 根据 AGPL-3.0，本项目不提供任何明示或暗示的担保
  5. **社区参与：** 欢迎社区的参与和贡献，我们鼓励开发者一同改进和维护本项目

<br />

## Star History

[![Star History Chart](https://api.star-history.com/chart?repos=Lrht-llw/KfuPet&type=date&legend=top-left)](https://www.star-history.com/?repos=Lrht-llw%2FKfuPet&type=date&legend=bottom-right)
